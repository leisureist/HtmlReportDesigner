/*globals rptEditor, svgCanvas, $, widget*/
...
*/
rptEditor.addExtension('storage', function() {
	// We could empty any already-set data for users when they decline storage,
	//  but it would be a risk for users who wanted to store but accidentally
	// said "no"; instead, we'll let those who already set it, delete it themselves;
	// to change, set the "emptyStorageOnDecline" config setting to true
	// in config.js.
	var emptyStorageOnDecline = rptEditor.curConfig.emptyStorageOnDecline,
		...
		noStorageOnLoad = rptEditor.curConfig.noStorageOnLoad,
		forceStorage = rptEditor.curConfig.forceStorage,
		storage = rptEditor.storage;
	...
	function setSVGContentStorage (val) {
		if (storage) {
			var name = 'svgedit-' + rptEditor.curConfig.canvasName;
			if (!val) {
	...
	function emptyStorage() {
		...
		var name;
		for (name in rptEditor.curPrefs) {
			if (rptEditor.curPrefs.hasOwnProperty(name)) {
				name = 'svg-edit-' + name;
				if (storage) {
					storage.removeItem(name);
				}
	...
	*/
	function setupBeforeUnloadListener () {
		window.addEventListener('beforeunload', function(e) {
			...
			rptEditor.setConfig({no_save_warning: true}); // No need for explicit saving at all once storage is on
			// rptEditor.showSaveWarning = false;

			var curPrefs = rptEditor.curPrefs;
			...
			if (!forceStorage && (
				...
				$.select(
					...
					function (pref, checked) {
						...
						//       rptEditor.setConfig({noStorageOnLoad: true});
						//   and then call:
						//       rptEditor.loadContentAndPrefs();
						
						// We don't check for noStorageOnLoad here because
						//   the prompt gives the user the option to store data
						setupBeforeUnloadListener();
						
						rptEditor.storagePromptClosed = true;
					},
					null,
					null,
					{

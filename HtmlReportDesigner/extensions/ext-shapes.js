...
rptEditor.addExtension('shapes', function() {'use strict';
	var current_d, cur_shape_id;
	var canv = rptEditor.canvas;
	var cur_shape;
	var start_x, start_y;
	...
	function loadLibrary(cat_id) {
		...
		if (!lib) {
			...
			$.getJSON(rptEditor.curConfig.extPath + 'shapelib/' + cat_id + '.json', function(result) {
	...
	return {
		svgicons: rptEditor.curConfig.extPath + 'ext-shapes.xml',
		buttons: [{
			id: 'tool_shapelib',

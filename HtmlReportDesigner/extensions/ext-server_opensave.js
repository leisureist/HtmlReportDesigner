/*globals rptEditor, svgedit, svgCanvas, canvg, $*/
...
rptEditor.addExtension("server_opensave", {
	callback: function() {
		'use strict';
		...
		var open_svg_action, import_svg_action, import_img_action,
			...
			save_svg_action = rptEditor.curConfig.extPath + 'filesave.php',
			save_img_action = rptEditor.curConfig.extPath + 'filesave.php',
		...
		open_svg_action = rptEditor.curConfig.extPath + 'fileopen.php?type=load_svg';
		import_svg_action = rptEditor.curConfig.extPath + 'fileopen.php?type=import_svg';
		import_img_action = rptEditor.curConfig.extPath + 'fileopen.php?type=import_img';
		
		// Set up function for PHP uploader to use
		rptEditor.processFile = function(str64, type) {
			var xmlstr;
			if (cancelled) {
				cancelled = false;
				return;
		...
		function rebuildInput(form) {
			...
			function submit() {
				// This submits the form, which returns the file data using rptEditor.processFile()
				form.submit();
			...
			if(form[0] == open_svg_form[0]) {
				inp.change(function() {
					...
					rptEditor.openPrep(function(ok) {
						if(!ok) {
							rebuildInput(form);
							return;
						}
